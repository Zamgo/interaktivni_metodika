---
title: Claim
typ: workflow
faze:
  - "realizace"
stav: draft
permalink: workflow/claim
navazane_cinnosti: []
---

## Účel workflow

Claim

## Kdy se spouští

Workflow se spouští, když [[Zhotovitel|zhotovitel]] uplatňuje nárok (claim) vůči [[Objednatel|objednateli]] dle smluvních podmínek FIDIC.

## Vstupy

- Dokumenty k předkládanému claimu (podklady, kalkulace, zdůvodnění nároku)

## Role

- [[Zhotovitel]]

## Hlavní kroky

| Krok | Název kroku | Odpovědná strana | Deadline | Popis kroku |
|---:|---|---|---|---|
| 1 | Oznámení Claimu | Zhotovitel |  | Odpovědná Role nahraje potřebné dokumenty k předkládanému Claimu. Posun do dalšího kroku. |
| 2 | Vyjádření se k podstatě Claimu | Zástupce objednatele | 42 dní | Odpovědná role se do 42dní vyjádří k podstatě Claimu. |

## Výstupy

- Vyjádření zástupce objednatele k podstatě claimu uložené v [[Společné datové prostředí (CDE)|CDE]]

## Vazba na CDE

Workflow je realizováno prostřednictvím CDE (schvalování, připomínkování, auditní stopa).

## Vazba na příslušnou přílohu nebo šablonu

- [[Šablony workflow (P1b)]]
- Ke stažení: [[00_Podklady/P1b_Šablony Workflow_Červený Fidic_prázdné.xlsx|P1b – Šablony workflow (Excel)]]
